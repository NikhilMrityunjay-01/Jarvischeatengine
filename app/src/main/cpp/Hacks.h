...
void DrawESP(ESP esp, int screenWidth, int screenHeight){
    // if(isESP) {
    esp.DrawText(Color(0, 255, 0, 255),"", //Green This signature alwayes show in your esp view, But easyly can chnage by hex editor throwg
    ...
    request.options.GWorld64 = 0xAA18A68;
    request.options.GWorld64VN = 0xAA1AB68;

    send((void*)&request,sizeof(request));
    receive((void*)&response);
    char hello[50]="failed";
    if(response.Success) {
        if (options.openState == 0 || options.bulletTrack == 0)
            ...
            if (response.Players[i].HeadLocation.z != 1) {
                ...
                if (x > -50 && x < screenWidth + 50) {//onScreen
                    ...
                    if (isSkelton && response.Players[i].Bone.isBone) {  //Skelton
                        esp.DrawLine(Color(0, 255, 0, 255), 2.1, Vec2(x, y),
                                     Vec2(response.Players[i].Bone.neck.x,
                                          response.Players[i].Bone.neck.y));
                        esp.DrawLine(Color(0, 255, 0, 255), 2.1,
                                     Vec2(response.Players[i].Bone.neck.x,
                                          response.Players[i].Bone.neck.y),
                        ...
                        esp.DrawLine(Color(0, 255, 0, 255), 2.1, Vec2(response.Players[i].Bone.lSh.x,
                                                                      response.Players[i].Bone.lSh.y),
                                     Vec2(response.Players[i].Bone.lElb.x,
                                          response.Players[i].Bone.lElb.y));
                        esp.DrawLine(Color(0, 255, 0, 255), 2.1, Vec2(response.Players[i].Bone.rSh.x,
                                                                      response.Players[i].Bone.rSh.y),
                        ...
                        esp.DrawLine(Color(0, 255, 0, 255), 2.1, Vec2(response.Players[i].Bone.lTh.x,
                                                                      response.Players[i].Bone.lTh.y),
                                     Vec2(response.Players[i].Bone.lKn.x,
                                          response.Players[i].Bone.lKn.y));
                        esp.DrawLine(Color(0, 255, 0, 255), 2.1, Vec2(response.Players[i].Bone.rTh.x,
                                                                      response.Players[i].Bone.rTh.y),
                                     Vec2(response.Players[i].Bone.rKn.x,
                                          response.Players[i].Bone.rKn.y));
                        esp.DrawLine(Color(0, 255, 0, 255), 2.1, Vec2(response.Players[i].Bone.lKn.x,
                                                                      response.Players[i].Bone.lKn.y),
                                     Vec2(response.Players[i].Bone.lAn.x,
                                          response.Players[i].Bone.lAn.y));
                        esp.DrawLine(Color(0, 255, 0, 255), 2.1, Vec2(response.Players[i].Bone.rKn.x,
                                                                      response.Players[i].Bone.rKn.y),
                    ...
                    //Player Box
                    if (isPlayerBox)
                        esp.DrawRect(Color(255, 255, 0), screenHeight / 700,
                                     Vec2(x - mx, top),
                                     Vec2(x + mx, bottom));
                    ...
                    if (isPlayerDist) {
                        ...
                        esp.DrawText(Color(255, 165, 0), extra,
                                     Vec2(x + 90, top - 28),textsize);
                    }
                    //weapon
                    if (isEnemyWeapon && response.Players[i].Weapon.isWeapon)
                        esp.DrawWeapon(Color(255, 0, 0), response.Players[i].Weapon.id,
       ...
       if (botCount + playerCount == 0) {
          ...
          if (botCount + playerCount > 0) { //This Name show red when exixst nearby enemy in your radar AntiRemove
              sprintf(extra, "", playerCount,botCount);
